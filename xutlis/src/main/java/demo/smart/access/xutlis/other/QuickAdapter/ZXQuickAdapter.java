 ...
 */
public abstract class ZXQuickAdapter<T, K extends ZXBaseHolder> extends RecyclerView.Adapter<K> {
    ...
    protected static final String TAG = ZXQuickAdapter.class.getSimpleName();
     ...
     */
    public ZXQuickAdapter(@LayoutRes int layoutResId, @Nullable List<T> data) {
    ...
    public ZXQuickAdapter(@Nullable List<T> data) {
        this(0, data);
    }

    public ZXQuickAdapter(@LayoutRes int layoutResId) {
    ...
    public void setOnItemClick(View v, int position) {
        getOnItemClickListener().onItemClick(ZXQuickAdapter.this, v, position);
    }
    ...
    protected K createBaseViewHolder(View view) {
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (K) new ZXBaseHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (K) new ZXBaseHolder(view);
    }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (ZXBaseHolder.class.isAssignableFrom(tempClass)) {
                        return tempClass;
                    }
                } else if (temp instanceof ParameterizedType) {
                    Type rawType = ((ParameterizedType) temp).getRawType();
                    if (rawType instanceof Class && ZXBaseHolder.class.isAssignableFrom((Class<?>) rawType)) {
    ...
    /**
     * When the current adapter is empty, the ZXQuickAdapter can display a special view
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, @IdRes int viewId) {
        ...
        ZXBaseHolder viewHolder = (ZXBaseHolder) recyclerView.findViewHolderForLayoutPosition(position);
        if (viewHolder == null) {
            return null;
        }
...
//            subItemCount += list.size();
    ...
    public interface OnItemChildClickListener {
         ...
         */
        void onItemChildClick(ZXQuickAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemChildLongClickListener {
         ...
         */
        boolean onItemChildLongClick(ZXQuickAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemLongClickListener {
         ...
         * @return true if the callback consumed the long click ,false otherwise
         */
        boolean onItemLongClick(ZXQuickAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemClickListener {
         ...
         */
        void onItemClick(ZXQuickAdapter adapter, View view, int position);
    }
