...
public class PhotoPickUtils {
    ...
    public static void onActivityResult(int requestCode, int resultCode, Intent data,PickHandler pickHandler ) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            }else if (requestCode == ZXPhotoPreview.REQUEST_CODE){//如果是预览与删除后返回
                if (data != null) {
                    ArrayList<String> photos = data.getStringArrayListExtra(PhotoPicker.KEY_SELECTED_PHOTOS);
    ...
    public static void startPick(Activity context, boolean showGif, int photoCount, ArrayList<String> photos, String viewId, boolean showCamera, boolean justCamera){
        PhotoPicker.builder()
                .setPhotoCount(photoCount)
                .setShowCamera(showCamera)
                .setShowGif(showGif)
                .setViewId(viewId)
                .setJustCamera(justCamera)
                .setSelected(photos)
