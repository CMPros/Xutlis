...
public class ZXDeviceUtil {
    ...
    private static boolean _isTablet() {
        return (ZXApp.getContext().getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK) >=
    ...
    public static boolean isFloatWindowOpAllowed() {
        ...
        if (version >= 19) {
            return checkOp(24);  // 24 是AppOpsManager.OP_SYSTEM_ALERT_WINDOW 的值，该值无法直接访问
        } else {
            try {
                return (ZXApp.getContext().getApplicationInfo().flags & 1 << 27) == 1 << 27;
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }
    ...
    private static boolean checkOp(int op) {
        ...
        if (version >= Build.VERSION_CODES.KITKAT) {
            AppOpsManager manager = (AppOpsManager) ZXApp.getContext().getSystemService(Context.APP_OPS_SERVICE);
            try {
                Method method = manager.getClass().getDeclaredMethod("checkOp", int.class, int.class, String.class);
                int property = (Integer) method.invoke(manager, op,
                        Binder.getCallingUid(), ZXApp.getContext().getPackageName());
