...
public class ZXLightUtil {
    ...
    public static boolean isAutoBrightnessEnabled() {
        try {
            int mode = Settings.System.getInt(
                    ZXApp.getContext().getContentResolver(),
    ...
    public static boolean setAutoBrightnessEnabled(final boolean enabled) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
                && !Settings.System.canWrite(ZXApp.getContext())) {
            Intent intent = new Intent(Settings.ACTION_MANAGE_WRITE_SETTINGS);
            intent.setData(Uri.parse("package:" + ZXApp.getContext().getPackageName()));
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            ZXApp.getContext().startActivity(intent);
            return false;
        }
    ...
    public static boolean setBrightness(@IntRange(from = 0, to = 255) final int brightness) {
        ...
        ContentResolver resolver = ZXApp.getContext().getContentResolver();
